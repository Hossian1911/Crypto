...
# 目标页面
SURF_STATS_URL = settings.SURF_STATS_URL
# 等待的元素（你提供的位置）
STATS_CONTAINER_XPATH = "/html/body/div[1]/div/main/div/div[2]/div[2]/div[2]/div"
...
def _build_driver(headless: bool = True) -> webdriver.Chrome:
    ...
    (function(){
      ...
      function shouldCapture(url){
        try {
          const u = String(url||'');
          // 捕获所有 surfv2-api*.surf.one 的请求（不限定路径），避免遗漏真正的交易对列表接口
          return /https?:\/\/surfv2-api[0-9\.\-]*\.surf\.one\//.test(u);
      ...
      Object.defineProperty(window, '__CAPTURED_SURF__', { value: __captured, writable: false });
    })();
    """
...
def _wait_for_stats_container(driver: webdriver.Chrome, timeout: float = 30.0) -> None:
    end = time.time() + timeout
    last_err: Exception | None = None
    while time.time() < end:
        try:
            el = driver.find_element(By.XPATH, STATS_CONTAINER_XPATH)
            if el and el.is_displayed():
                return
        except Exception as e:
            last_err = e
        time.sleep(0.5)
    raise TimeoutException(f"等待 stats 容器渲染超时: {last_err}")
...
def run_probe(headless: bool = True, url: str | None = None) -> Path:
    out_base = settings.DATAGET_OUTPUT_DIR / "surf" / "probe"
    out_dir = out_base / time.strftime("%Y%m%d_%H%M%S")
    driver = _build_driver(headless=headless)
    try:
        target = url or SURF_STATS_URL
        driver.set_page_load_timeout(60)
        driver.get(target)
        _wait_for_stats_container(driver, timeout=30)
        # 等待页面触发相关请求
        time.sleep(3)
        resources = _collect_resources(driver)
        captured = driver.execute_script("return window.__CAPTURED_SURF__ || { requests: [], resources: [], initial: null };")
        # 保存页面 HTML 以排查 SSR
        try:
